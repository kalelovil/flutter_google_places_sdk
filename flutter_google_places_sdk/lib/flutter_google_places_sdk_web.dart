...
import 'package:google_maps/google_maps_places.dart' as places2;
...
class FlutterGooglePlacesSdkWebPlugin extends FlutterGooglePlacesSdkPlatform {
  ...
  places2.AutocompleteService? _svcAutoComplete;
  places2.PlacesService? _svcPlaces;
  places2.AutocompleteSessionToken? _lastSessionToken;

  // Language
  String? _language;

  // Cache for photos
  final _photosCache = <String, places2.PlacePhoto>{};
  var _runningUid = 1;
  ...
  void _doInit() {
    _svcAutoComplete = places2.AutocompleteService();
    _svcPlaces =
        places2.PlacesService(html.window.document.createElement('div'));
    _completer!.complete();
  }
  ...
  Future<FindAutocompletePredictionsResponse> findAutocompletePredictions(
    ...
    final prom =
        _svcAutoComplete!.getPlacePredictions(places2.AutocompletionRequest()
          ..input = query
          ..origin = origin == null ? null : core.LatLng(origin.lat, origin.lng)
          ..types = placeTypesFilter.isEmpty ? null : placeTypesFilter
          ..componentRestrictions =
              (places2.ComponentRestrictions()..country = countries)
  ...
  AutocompletePrediction _translatePrediction(
      places2.AutocompletePrediction prediction) {
  ...
  Future<FetchPlaceResponse> fetchPlace(
    ...
    final prom = _getDetails(places2.PlaceDetailsRequest()
      ..placeId = placeId
  ...
  Future<NearbySearchResponse> nearbySearch(
    ...
    final prom = _nearbySearch(places2.PlaceSearchRequest()
  ...
  Future<_NearbySearchResponse> _nearbySearch(
      places2.PlaceSearchRequest request) {
    final completer = Completer<_NearbySearchResponse>();

    final func = (List<places2.PlaceResult?>? places,
        places2.PlacesServiceStatus? status,
        places2.PlaceSearchPagination? pages) {
  ...
  Future<_GetDetailsResponse> _getDetails(places2.PlaceDetailsRequest request) {
    final completer = Completer<_GetDetailsResponse>();

    final func =
        (places2.PlaceResult? place, places2.PlacesServiceStatus? status) {
  ...
  List<Place?>? _parsePlaces(List<places2.PlaceResult?>? places) {
    if (places == null) {
      return null;
    }
    ...
    for (int i = 0; i < places.length; i++) {
      Place? parsedPlace = _parsePlace(places[i]);
      parsedPlaces.add(parsedPlace);
    }
    return parsedPlaces;
  }

  Place? _parsePlace(places2.PlaceResult? place) {
    if (place == null) {
      return null;
    }
  ...
  AddressComponent? _parseAddressComponent(
      core.GeocoderAddressComponent? addressComponent) {
  ...
  LatLng? _parseLatLang(core.LatLng? location) {
    if (location == null) {
      return null;
    }
  ...
  core.LatLng? _parseLatLang2(LatLng? location) {
    if (location == null) {
      return null;
    }
  ...
  PhotoMetadata? _parsePhotoMetadata(places2.PlacePhoto? photo) {
    if (photo == null) {
      return null;
    }
  ...
  String _getPhotoMetadataReference(places2.PlacePhoto photo) {
    final num = _runningUid++;
    return "id_${num.toString()}";
  }

  LatLngBounds? _parseLatLngBounds(core.LatLngBounds? viewport) {
    if (viewport == null) {
      return null;
    }
  ...
  PlusCode? _parsePlusCode(places2.PlacePlusCode? plusCode) {
    if (plusCode == null) {
      return null;
    }
  ...
  BusinessStatus? _parseBusinessStatus(String? businessStatus) {
    ...
    return BusinessStatus.values.firstWhereOrNull(
        (element) => element.name.toUpperCase() == businessStatus);
  }

  OpeningHours? _parseOpeningHours(places2.PlaceOpeningHours? openingHours) {
  ...
  Period _parsePeriod(places2.PlaceOpeningHoursPeriod period) {
    return Period(
        open: _parseTimeOfWeek(period.open)!,
        close: _parseTimeOfWeek(period.close));
  }

  TimeOfWeek? _parseTimeOfWeek(places2.PlaceOpeningHoursTime? timeOfWeek) {
  ...
  core.LatLngBounds? _boundsToWeb(LatLngBounds? bounds) {
    if (bounds == null) {
      return null;
    }
  ...
  core.LatLng _latLngToWeb(LatLng latLng) {
  ...
  Future<FetchPlacePhotoResponse> fetchPlacePhoto(
    ...
    places2.PlacePhoto? value = _photosCache[photoMetadata.photoReference];
    if (value == null) {
    ...
    final options = places2.PhotoOptions()
      ..maxWidth = maxWidth
      ..maxHeight = maxHeight;
...
class _GetDetailsResponse {
  ...
  final places2.PlacesServiceStatus? status;
}
